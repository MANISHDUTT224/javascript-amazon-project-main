...
localStorage.setItem('cart-business',JSON.stringify(cart1));
 ...
 class CartGenerator{
    ...
    // # means that the particular attribute or method is declared private(inaccessible outside the class)
    ...
    saveTostorage(){
        localStorage.setItem(this.#localStoragekey,JSON.stringify(Cart.cart));
      }
      addtocart(productId){
        let matchitem;
...
const Cart=new CartGenerator('cart-oops');
const BusinessCart=new CartGenerator('cart-business');
